...
def filter(pred, seq):
    ...
    [2, 4]
    """
    return [elem for elem in seq if pred(elem) == True]
...
def reduce(combiner, seq):
    ...
    while index < len(seq) - 1:
        total = combiner(total, seq[index + 1])
        index += 1
    return total
...
def add_song(t, song, category):
    ...
    new_branhces = []
    for b in branches(t):
        new_branhces += [add_song(b, song, category)]
        return tree(root(t), new_branhces)

    # else:
    #     return sum([num_songs(b) for b in branches(t)])


    # return tree(root(t), b for b in branches(t).append(song) if b == song)
