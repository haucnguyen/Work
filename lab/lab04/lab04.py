...
def distance(city1, city2):
    ...
    c1 = city1
    c2 = city2
    if get_lat(c1) == get_lat(c2):
        return sqrt(abs(get_lon(c1) - get_lon(c2)))
    else:
        return sqrt((get_lon(c1) - get_lon(c2))**2 + (get_lat(c1) - get_lat(c2))**2)
...
def replace_elem(lst, index, elem):
    ...
    return [lst[_] if _ != index else elem for _ in range(len(lst))]
...
def put_piece(board, max_rows, column, player):
    ...
    """
    row_i = max_rows - 1
    succesful_placement = False
    while row_i >= 0 and succesful_placement == False:
        if get_piece(board, row_i, column) == '-':
            board[row_i][column] = player
            succesful_placement = True
            row_p = row_i
        row_i -= 1
    if succesful_placement:
        return row_p, board
    else:
        return -1, board
