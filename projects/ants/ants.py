    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 7
                """"self can an ant or body guard, insect can be ant or
                body guard too so check for both"""
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 6A
        if self.place.ant == None or self.place.ant.blocks_path:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place.entrance is not hive and not current_place.bees and transitions_counter < self.max_range:
            current_place = current_place.entrance
...
class Water(Place):
    """Water is a place that can only hold 'f' insects."""
...
class WallAnt(Ant):# The WallAnt class
    """WallAnt does nothing but block with armor"""
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6A
        bees_in_spot = self.place.bees[:]
        for b in bees_in_spot:
            b.reduce_armor(self.damage)
        # END Problem 6A
...
class ScubaThrower(ThrowerAnt):
    """can be placed in water and does not lose armor, scuba can throw on land"""
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    implemented = False   # Change to True to view in the GUI
    impostor = False
    existing_queens = False
    damage_doubled = [] #keep track of ants whose damage were doubled
    # END Problem 9

    def __init__(self,):
        # BEGIN Problem 9
        if self.existing_queens:
            self.impostor = True
        QueenAnt.existing_queens = True
        # END Problem 9
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            while next_exit != None:
                exits.append(next_exit)
                next_exit = next_exit.exit
            for place in exits:
                if place.ant != None:
                    if place.ant not in QueenAnt.damage_doubled:
                        place.ant.damage = place.ant.damage * 2
                        QueenAnt.damage_doubled.append(place.ant)
                    if place.ant.container and place.ant.ant is not None and place.ant.ant not in QueenAnt.damage_doubled:
                        place.ant.ant.damage = place.ant.ant.damage * 2
                        QueenAnt.damage_doubled.append(place.ant.ant)
...
def apply_effect(effect, bee, duration):
    ...
    new_action = effect(old_action, duration)
