...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        arguments = expr.second.map(lambda operand: scheme_eval(operand, env))
        procedures = scheme_eval(expr.first, env)
        return scheme_apply(procedures, arguments, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('number of args not equal number for parameters')
        else:
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            if not scheme_procedurep(procedure):
                raise SchemeError("cannot call: {0}".format(str(procedure)))
            result = procedure.evaluate_all(rest, env)
            # END Extra Credit
    return result
